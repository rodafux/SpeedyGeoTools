...
function generateAndPrint() {
    ...
    const stripsPerPage = Math.floor(190 / stripWidth);
        ...
        for (let i = 0; i < stripsPerPage; i++) {
            ...
            if (stripCounter === 0) {
                ...
                if (!isEmergency && hasQrCode && qrUrl !== '') {
                    ...
                    const qrSize = is25mm ? 60 : 90;
                    new QRious({ element: qrCanvas, value: finalQrUrl, size: qrSize });
                ...
                if (!isEmergency && !podiumPrinted) {
                    podiumPrinted = true;
                    if (hasFtf) {
                        const ftfLine = document.createElement('div');
                        ftfLine.className = hasFtfTampon ? 'podium-stamp with-tampon' : 'podium-stamp';
                        ftfLine.innerHTML = `<div class="podium-header"><span class="podium-title">FTF</span></div>${hasFtfTampon ? `<div class="podium-tampon">${t.tampon || "Tampon"}</div>` : ''}`;
                        strip.appendChild(ftfLine);
                    }
                    if (hasStfTtf) {
                        const stfLine = document.createElement('div');
                        stfLine.className = hasStfTtfTampon ? 'podium-stamp with-tampon' : 'podium-stamp';
                        stfLine.innerHTML = `<div class="podium-header"><span class="podium-title">STF</span></div>${hasStfTtfTampon ? `<div class="podium-tampon">${t.tampon || "Tampon"}</div>` : ''}`;
                        strip.appendChild(stfLine);
                        const ttfLine = document.createElement('div');
                        ttfLine.className = hasStfTtfTampon ? 'podium-stamp with-tampon' : 'podium-stamp';
                        ttfLine.innerHTML = `<div class="podium-header"><span class="podium-title">TTF</span></div>${hasStfTtfTampon ? `<div class="podium-tampon">${t.tampon || "Tampon"}</div>` : ''}`;
                        strip.appendChild(ttfLine);
                    }
                }
