...
function exportLogbookConfig() {
    ...
    a.href = dataStr;
    a.download = `speedygeotools_logbook_${new Date().toISOString().slice(0, 10)}.json`;
...
function importLogbookConfig(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            event.target.value = '';
            alert("Configuration Logbook chargée ! (Attention: Le logo personnalisé doit être rechargé manuellement)");
        } catch (err) {
